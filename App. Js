...
async function loadOffcuts() {
  ...
  const { data, error } = await supabase.from("offcut_inventory").select("*").order("created_at", { ascending: false });
  if (error) {
...
function renderOffcutTable() {
  ...
  tbody.querySelectorAll(".del-btn").forEach(btn => {
    btn.addEventListener("click", async (e) => {
      ...
      await supabase.from("offcut_inventory").delete().eq("id", e.target.dataset.id);
      loadOffcuts();
    });
  });
}
...
document.getElementById("manAddBtn").addEventListener("click", async () => {
  ...
  if (!supabase) { setMsg("manAddMsg", "Supabase not configured.", "error"); return; }
  const { error } = await supabase.from("offcut_inventory").insert([{ profile, grade, length_mm, note }]);
...
document.getElementById("commitBtn").addEventListener("click", async () => {
  ...
  if (!supabase) { setMsg("commitMsg", "Supabase not configured — nothing to commit.", "error"); return; }
  ...
  try {
    ...
    if (nestResult.consumedOffcutIds.length > 0) {
      await supabase.from("offcut_inventory").delete().in("id", nestResult.consumedOffcutIds);
    }
    const toInsert = [
    ...
    if (toInsert.length > 0) {
      await supabase.from("offcut_inventory").insert(toInsert);
    }
